...
export class StateManager {
    ...
    Object.entries(json).forEach(([pageKey, items]) => {
      ...
      items.forEach(item => {
        ...
        this.overlayData[pageKey][key] = {
          ...
          type,
          originalBbox: bbox,
          // ✨ NEW: Store original content for tables
  ...
  _formatContent(content, type) {
    ...
    const formatters = {
      ...
      // ✨ FIXED: Store table as JSON to preserve structure
      [CONFIG.CONTENT_TYPES.TABLE]: () => {
        if (!Array.isArray(content)) return String(content || '');
        // Store as JSON - we'll parse it when rendering
  ...
  updateOverlayText(pageNum, coords, text) {
    ...
    if (this.overlayData[pageKey]?.[coords]) {
      ...
      // ✨ NEW: Update tableData if it's a table
      if (this.overlayData[pageKey][coords].type === CONFIG.CONTENT_TYPES.TABLE) {
        try {
  ...
  getOverlayType(pageNum, coords) {
    const pageKey = `page_${pageNum}`;
    return this.overlayData[pageKey]?.[coords]?.type || CONFIG.CONTENT_TYPES.TEXT;
  }
