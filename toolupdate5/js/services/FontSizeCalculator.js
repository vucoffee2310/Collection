...
export class FontSizeCalculator {
  ...
  calculateOptimalSize(overlay) {
    ...
    const w = overlay.clientWidth,
      h = parseFloat(overlay.dataset.targetHeight) || overlay.clientHeight;
    if (w <= 0 || h <= 0) return;
  ...
  _fitTable(overlay) {
    const box = overlay.querySelector(".overlay-text"),
      table = box?.querySelector(".data-table");
    if (!box || !table) return;
    ...
    const rows = table.rows.length,
      len = (table.textContent || "").length || 1;
    ...
    const fits = (size) => {
      ...
      if (equalRows) this._equalizeRows(table, box);
      else this._clearRowHeights(table);
    ...
    if (this.cache.has(key)) {
      box.style.fontSize = toPx(this.cache.get(key));
      box.offsetHeight;
      if (equalRows) this._equalizeRows(table, box);
    ...
    let guess = Math.max(
      MIN,
      Math.min(MAX, Math.sqrt((w * h) / Math.max(len, 1)) * 1.6),
    );
  ...
  _autoCols(table) {
    ...
    rows.forEach((r) => {
      Array.from(r.children).forEach((cell, j) => {
        ...
        scores[j] = Math.max(
          scores[j],
          maxLine * (cell.tagName === "TH" ? HEAD_BOOST : 1),
        );
      });
    });
    ...
    if (remain === 0) {
      perc = new Array(cols).fill(100 / cols);
    } else {
      let freeSum = 0;
      const freeIdx = [];
      for (let i = 0; i < cols; i++)
      ...
      if (!freeIdx.length) perc = new Array(cols).fill(100 / cols);
        ...
        freeIdx.forEach((i) => (perc[i] = remain * (base[i] / (freeSum || 1))));
    }
    let excess = 0,
      flex = [];
    for (let i = 0; i < cols; i++) {
      if (perc[i] > maxPct) {
        excess += perc[i] - maxPct;
        perc[i] = maxPct;
      } else flex.push(i);
    }
    if (excess > 0 && flex.length) {
      const headroom = flex.map((i) => maxPct - perc[i]),
        hrSum = headroom.reduce((a, b) => a + b, 0) || 1;
      flex.forEach((i, k) => (perc[i] += excess * (headroom[k] / hrSum)));
    }
    const norm = 100 / (perc.reduce((a, b) => a + b, 0) || 1);
    perc = perc.map((p) => p * norm);
    ...
    for (let i = 0; i < cols; i++)
      cg.children[i].style.width = `${perc[i].toFixed(4)}%`;
  }
  ...
  _clearRowHeights(table) {
    table.querySelectorAll("tr,th,td").forEach((el) => (el.style.height = ""));
  }
  ...
  _exaggerate(table, on) {
    ...
    cells.forEach((cell) => {
      ...
      const aw = Math.max(0, cell.clientWidth - padX),
        ah = Math.max(0, cell.clientHeight - padY);
      wrap.style.display = "inline-block";
      wrap.style.transformOrigin = "top left";
      wrap.style.transform = "scale(1)";
      cell.style.overflow = "hidden";
      wrap.offsetHeight;
      const nw = wrap.offsetWidth || 0.001,
        nh = wrap.offsetHeight || 0.001;
      let s =
        nw > 0 && nh > 0 && aw > 0 && ah > 0
          ? Math.min(aw / nw, ah / nh) * 0.995
          : 1;
