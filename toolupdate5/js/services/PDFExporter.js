...
export class PDFExporter {
  ...
  _getLinePositionsWithSpacing(element, wrapperRect) {
    ...
    const baselineRatio = this._measureBaselineRatio(
      style.fontFamily,
  ...
  _drawVerticalText(pdf, o) {
    ...
    if (Math.abs(o.letterSpacing) > 0.1) {
      let currentY =
        centerY - (text.length * (o.fontSize + o.letterSpacing)) / 2;
  ...
  _drawList(pdf, o) {
    ...
    items.forEach((item) => {
      pdf.text(bulletChar, o.x + o.pad[1], currentY, {
        baseline: "alphabetic",
      });
      ...
      lines.forEach((line, idx) => {
        ...
        pdf.text(line, xPos, currentY + idx * lineSpacing, {
          baseline: "alphabetic",
        });
      });
