...
export class OverlayRenderer {
  constructor(state, fontCalc){ this.state = state; this.fontCalc = fontCalc; }

  renderPageOverlays(wrapper, page, dims, data){
    const pageData = data[`page_${page}`]; if (!pageData) return;
    ...
    requestAnimationFrame(() => queue.forEach(o => this.fontCalc.calculateOptimalSize(o)));
  }
  ...
  _createOverlay(coords, info, page, dims, coordOrder){
    ...
    const isVertical = type === CONFIG.CONTENT_TYPES.TEXT && pos.width > 0 && (pos.height / pos.width) > CONFIG.OVERLAY.VERTICAL_THRESHOLD;
    const isSingle = type === CONFIG.CONTENT_TYPES.TEXT && !info.text.includes('<div') && !info.text.includes('\n');

    const overlay = document.createElement('div');
    Object.assign(overlay.dataset, { coords, pageNum: page, targetHeight: pos.height, contentType: type });
    overlay.className = ['overlay', `content-${type}`, isVertical ? 'vertical-text' : '', isSingle ? 'single-line-layout' : ''].filter(Boolean).join(' ');
  ...
  _createTextElement(info){
    ...
    txt.addEventListener('blur', debounce(e => {
      const overlay = e.target.closest('.overlay'); if (!overlay) return;
      const t = overlay.dataset.contentType;
      let val;
      if (t === CONFIG.CONTENT_TYPES.TABLE) val = this._extractTableToJSON(e.target);
      else if (t === CONFIG.CONTENT_TYPES.CODE) val = e.target.textContent;
      else if (e.target.querySelector('.merged-text-block')) val = e.target.innerHTML;
      else val = e.target.innerText;
      this.state.updateOverlayText(overlay.dataset.pageNum, overlay.dataset.coords, val);
      if ([CONFIG.CONTENT_TYPES.TEXT, CONFIG.CONTENT_TYPES.LIST, CONFIG.CONTENT_TYPES.CODE, CONFIG.CONTENT_TYPES.TABLE].includes(t)) {
  ...
  _formatContent(info){
    ...
    const map = {
      ...
      [CONFIG.CONTENT_TYPES.LIST]: () => text.includes('<div class="list-item">')
        ? text
        : text.split('\n').filter(l => l.trim()).map(l => `<div class="list-item">${escapeHtml(l)}</div>`).join(''),
      [CONFIG.CONTENT_TYPES.TABLE]: () => this._formatTableFlow(text, info.tableData),
      [CONFIG.CONTENT_TYPES.IMAGE]: () => '<div class="image-placeholder">[Image]</div>',
      [CONFIG.CONTENT_TYPES.TEXT]: () => text.includes('<div class="merged-text-block">') ? text : escapeHtml(text || '')
    };
  ...
  _formatTableFlow(text, tableData){
    let data = tableData;
    if (!data) { try { data = JSON.parse(text); } catch { return text; } }
    if (!Array.isArray(data) || !data.length) return text;

    const rows = data.filter(r => Array.isArray(r)); if (!rows.length) return text;
    ...
    let perc = (() => {
      ...
      const fixed = new Set(); let fixedSum = 0; const base = p.slice();

      for (let i=0;i<cols;i++){ if (base[i] < minPct){ p[i] = minPct; fixed.add(i); fixedSum += minPct; } }
      const remain = Math.max(0, 100 - fixedSum);
      if (remain === 0) { p = new Array(cols).fill(100/cols); }
      else {
        ...
        else freeIdx.forEach(i => p[i] = remain * (base[i]/(freeSum||1)));
      }
      let excess = 0, flex = [];
      for (let i=0;i<cols;i++){ if (p[i] > maxPct){ excess += p[i] - maxPct; p[i] = maxPct; } else flex.push(i); }
      if (excess > 0 && flex.length){
        const headroom = flex.map(i => maxPct - p[i]), sumH = headroom.reduce((a,b)=>a+b,0) || 1;
    ...
    const cell = (tag, c) => `<${tag}>${escapeHtml(String(c ?? '')).replace(/\n/g,'<br>') || '&nbsp;'}</${tag}>`;
    const head = `<thead><tr>${(rows[0] || []).map(c => cell('th', c)).join('')}</tr></thead>`;
    const body = `<tbody>${rows.slice(1).map(r => `<tr>${Array.from({length: cols}, (_, j) => cell('td', r[j] ?? '')).join('')}</tr>`).join('')}</tbody>`;
  ...
  _extractTableToJSON(root){
    const t = root.querySelector('.data-table'); if (!t) return '[]';
