...
export const cleanupNestedBrackets = (text) => {
  ...
  let cleaned = text.replace(
    new RegExp(`${COMPOUND_OPEN}{2,}`, "g"),
    COMPOUND_OPEN
  );
  cleaned = cleaned.replace(
    new RegExp(`${COMPOUND_CLOSE}{2,}`, "g"),
    COMPOUND_CLOSE
  );

  const openCount = (cleaned.match(new RegExp(COMPOUND_OPEN, "g")) || [])
    .length;
  const closeCount = (cleaned.match(new RegExp(COMPOUND_CLOSE, "g")) || [])
    .length;

  if (openCount !== closeCount) {
    console.warn(
      `⚠️ Mismatched brackets: ${openCount} open, ${closeCount} close`
    );
  }
...
export const safelyMergeCompounds = (text, force = false) => {
  ...
  const alreadyHasMarkers =
    text.includes(COMPOUND_OPEN) && text.includes(COMPOUND_CLOSE);
...
export const fixCompoundBoundaries = (segments) => {
  ...
  for (let i = 0; i < adjusted.length - 1; i++) {
    ...
    console.log(
      `🔗 Compound boundary fix: "${lastWord}" + "${firstWord}" → «${lastWord} ${firstWord}»`
    );
