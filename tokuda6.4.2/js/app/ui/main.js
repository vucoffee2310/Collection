...
export const createUI = (tracks) => {
  ...
  if (!tracks?.length) {
    container.innerHTML =
      '<div style="padding: 10px; background: #f0f0f0; border: 1px solid #ccc; color: #000; font-size: 12px; border-radius: 4px;">No subtitles available</div>';
  } else {
    ...
    if (tracks.length > 1) {
      ...
      const getTrackLabel = (track) =>
        ...
        track?.languageCode ||
        "Unknown";
      info.textContent = `Available languages: ${tracks
        .map((t) => getTrackLabel(t))
        .join(", ")}`;
