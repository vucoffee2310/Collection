...
(function () {
  ...
  const waitFor = async (
    selector,
    { all = false, retries = 20, interval = 500 } = {}
  ) => {
    ...
    throw new Error(
      `Element(s) not found after ${retries} retries: ${selector}`
    );
  };
  ...
  const monitorResponse = async (expectedMarkerCount) => {
    console.log(
      `🔍 Starting response monitoring, expecting ${expectedMarkerCount} markers.`
    );
    ...
    while (unchangedChecks < maxUnchangedChecks) {
      ...
      if (currentText && currentText !== previousText) {
        ...
        console.log(
          `📝 Text updated: ${currentText.length} chars, ${markerCount}/${expectedMarkerCount} markers`
        );
    ...
    console.log(
      `✅ Response complete: ${previousText.length} chars, ${finalMarkerCount} markers`
    );
  ...
  const runAutomation = async (promptText, cardName) => {
    try {
      ...
      const sliders = await waitFor(
        ".mat-mdc-slider.mdc-slider input[type='range']",
        { all: true }
      );
      ...
      if (sliders[0]) {
        setSlider(sliders[0], 1.5);
        await wait(500);
      }
      if (sliders[1]) {
        setSlider(sliders[1], 300);
        await wait(500);
      }
      if (sliders[2]) {
        setSlider(sliders[2], 0.5);
        await wait(500);
      }
