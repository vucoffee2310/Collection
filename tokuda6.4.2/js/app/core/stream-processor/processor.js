...
export class StreamingTranslationProcessor {
  ...
  rebuildUnmatchedMap() {
    ...
    Object.entries(this.sourceJSON.markers).forEach(
      ([markerKey, instances]) => {
      ...
      if (lastMatch) {
        ...
        if (this.events.length < this.maxEvents) {
          this.events.push({
            ...
            content:
              this.currentMarker.content.substring(0, 50) +
              (this.currentMarker.content.length > 50 ? "..." : ""),
