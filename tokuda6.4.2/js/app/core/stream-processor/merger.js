...
export const checkForOrphans = (
  processor,
  beforePosition,
  currentMatchedInstance = null
) => {
...
const handleLeadingOrphans = (processor, orphans) => {
  ...
  console.log(
    `👑 Leading orphan group: ${leader.domainIndex} (#${leader.position})`
  );
  ...
  for (let i = 1; i < orphans.length; i++) {
    ...
    leader.groupMembers.push({
      ...
      totalUtteranceWords:
        orphan.utterances?.reduce((sum, u) => sum + (u.wordLength || 0), 0) ||
        0,
    });
    ...
    console.log(
      `  ↳ Merged ${orphan.domainIndex} (#${orphan.position}) into orphan group`
    );
  }
};
...
const handleTrailingOrphans = (processor, orphans) => {
  ...
  console.log(
    `👑 Trailing orphan group: ${leader.domainIndex} (#${leader.position})`
  );
...
export const mergeOrphanToPreceding = (
  processor,
  orphanInstance,
  precedingMatch = null
) => {
  ...
  if (!targetMatch) {
    ...
    console.log(
      `⚠️ Orphan: ${orphanInstance.domainIndex} (#${orphanInstance.position})`
    );
    return;
  }
  ...
  targetMatch.mergedOrphans.push({
    ...
    totalUtteranceWords:
      orphanInstance.utterances?.reduce(
...
export const mergeOrphanForward = (
  processor,
  orphanInstance,
  targetInstance
) => {
  ...
  if (
    targetInstance.overallLength !== undefined &&
    orphanInstance.overallLength !== undefined
  ) {
  ...
  console.log(
    `🔗 Forward: ${orphanInstance.domainIndex} (#${orphanInstance.position}) → ${targetInstance.domainIndex}`
  );
};
