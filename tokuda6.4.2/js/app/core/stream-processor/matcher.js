...
export const matchAndUpdate = (processor, transMarker) => {
  ...
  const getPrev = (count) => {
    ...
    return processor.completedMarkers
      .slice(len - 1 - count, len - 1)
      .map((m) => m.marker);
  };
  ...
  if (!processor[lookupKey]) {
    processor[lookupKey] = buildPrevLookupMap(
      unmatchedInstances,
      processor.sourcePrevCache
    );
  }
    ...
    if (sourceInstance.edgeCase && trans.edgeCase) {
      for (let j = 0; j <= markerIndex; j++) {
        ...
        if (
          ...
          if (
            JSON.stringify(sourceInstance[prevKey]) ===
            JSON.stringify(trans[prevKey])
          ) {
  ...
  if (!matched && unmatchedInstances.length > 0) {
    ...
    if (!bestCandidate) {
      bestCandidate = unmatchedInstances.sort(
  ...
  if (matched && matchedInstance) {
    ...
    const remaining = unmatchedInstances.filter(
      (inst) => inst !== matchedInstance
    );
