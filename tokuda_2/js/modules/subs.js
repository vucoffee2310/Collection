import { CONFIG, SPLIT_DELIMITER } from './config.js';
import { SeededRandom } from './rng.js';
import { countWords } from './utils.js';
import { mergeLowMarkerParagraphs, splitTextAtMiddle } from './text.js';
...
export const parseSubtitlesWithTiming = (xml, lang = 'en') => {
  const textElements = Array.from(new DOMParser().parseFromString(xml, 'text/xml').getElementsByTagName('text'));
...
export const convertSubtitlesToMarkedParagraphs = (xml, lang = 'en') => {
  ...
  for (let i = 0; i < utterances.length;) {
    const segCount = rng.nextInt(CONFIG.MIN_MARKERS_PER_PARAGRAPH, CONFIG.MAX_MARKERS_PER_PARAGRAPH);
    const segments = [];
    ...
    if (segments.length) {
      const longest = new Set(segments.slice().sort((a, b) => b.text.length - a.text.length)
        .slice(0, CONFIG.SPLIT_COUNT).map(s => s.idx));
  ...
  const finalText = mergeLowMarkerParagraphs(paragraphs.join('\n\n').replace(new RegExp(SPLIT_DELIMITER, 'g'), '\n\n'));
