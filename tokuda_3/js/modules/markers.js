...
import { formatSRTTime, formatVTTTime } from './ui/helpers.js';
...
const detectLanguage = (text) => {
  ...
  if (cjkPattern.test(text)) {
    ...
    if (/[\u3040-\u309F]/.test(text)) return 'ja'; // Hiragana = Japanese
    if (/[\u30A0-\u30FF]/.test(text)) return 'ja'; // Katakana = Japanese
    return 'zh'; // Otherwise Chinese
  }
...
export const extractMarkersWithContext = (text, utteranceData = []) => {
  ...
  for (let m; (m = pattern.exec(clean));) {
    positions.push({ letter: m[1], start: m.index, end: m.index + m[0].length });
  }
  ...
  markers.forEach((m, i) => {
    if (i < 3) {
      ...
      for (let j = 0; j <= i; j++) m[`prev${j}`] = getPrev(i, j);
    } else {
      const [p5, p4, p3] = [getPrev(i, 5), getPrev(i, 4), getPrev(i, 3)];
      Object.assign(m, {
        prev5: p5, prev4: p4, prev3: p3,
