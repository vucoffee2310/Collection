 ...
 */
import { debounce, throttle, escapeHtml } from "../../../shared/js/utils.js"; // ✅ Import directly from shared
import {
  createWaitingEvent,
  createMatchedEvent,
  createOrphanEvent,
} from "./debugger-events.js";
import {
  renderDataTable,
  renderFlow,
  renderComparison,
...
export class ProcessDebugger {
  constructor() {
    ...
    this.debouncedBreakdownRender = debounce(
  ...
  handleSourceSegments(segments) {
    console.log(
      "[Process Debugger] 📥 Received source segments:",
      segments.length
    );
  ...
  processQueue() {
    ...
    while (
      ...
      const sourceMatch = this.sourceSegments.find(
        (s) => s.marker === segment.marker
      );
    ...
    if (this.eventQueue.length > 0) {
      const delay =
        this.isSyncingHistorical && this.eventQueue.length > 50 ? 100 : 0;
      setTimeout(() => {
        requestAnimationFrame(() => this.processQueue());
      }, delay);
    } else {
  ...
  renderMarkerBreakdown() {
    ...
    if (this.sourceSegments.length === 0) {
      container.innerHTML =
        '<div class="breakdown-empty">No source segments loaded yet. Start from the main page.</div>';
      return;
    }
    ...
    const gridHtml = `<div class="breakdown-grid">${allMarkers
      .map(
        (item) =>
    ...
    if (waitingMarkers.length > 0) {
      waitingListHtml = `<div class="waiting-list">
                <strong>⏳ ${
                  waitingMarkers.length
                } marker(s) still waiting for AI response:</strong>
  ...
  showTimelineForMarker(marker) {
    ...
    let headerHtml = `<h2 class="timeline-header">
            <span>${marker} Timeline <span class="timeline-event-count">(${
      events.length
    } event${events.length > 1 ? "s" : ""})</span></span>
    ...
    this.timelineContainer.scrollIntoView({
      behavior: "smooth",
      block: "start",
    });
  }
                ...
                <div class="event severity-${severityClass}">
                    ...
                    <div class="event-content">
                        ...
                        ${
                          eventData.statusText
                            ? `<div class="status-text">${eventData.statusText.replace(
                                "Status: ",
                                ""
                              )}</div>`
                            : ""
                    ...
                    <div class="event-details">
                        <div class="details-title">
                            🔍 ${
                              eventData.type === "orphan"
                                ? "ORPHAN"
                                : eventData.type === "waiting"
                                ? "WAITING"
                                : "MATCHED"
                            } Processing Details
                        </div>
  ...
  renderSteps(steps) {
    return steps
      .map((step, i) => {
        ...
        }${step.alert ? renderAlert(step.alert) : ""}</div>`;
  ...
  saveLog() {
    try {
      ...
      if (sizeInMB > 5) {
        if (
          !confirm(
            `Large log file (${sizeInMB.toFixed(1)} MB). Continue download?`
          )
        ) {
          return;
        }
      ...
      console.log(
        "[Process Debugger] 💾 Saved full log:",
  ...
  buildMetadata() {
    return {
      ...
      sessionDuration:
        ((Date.now() - this.sessionStartTime) / 1000).toFixed(1) + "s",
  ...
  saveMarkerLog(marker, status) {
    try {
      ...
      const logData = {
        ...
        target: target
          ? { marker: target.marker, text: target.text, exists: true }
          : { exists: false },
        status: status,
        events: events,
        statistics: {
