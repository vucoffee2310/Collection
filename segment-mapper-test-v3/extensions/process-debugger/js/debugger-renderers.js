...
import { escapeHtml } from "../../../shared/js/utils.js"; // ✅ Import directly from shared
import { formatJSON } from "../../../shared/js/formatters.js"; // ✅ Import directly from shared

export function renderDataTable(data) {
  let html = '<table class="data-table">';
  Object.keys(data).forEach((key) => {
    html += `<tr><th>${key}</th><td class="mono">${escapeHtml(
      String(data[key])
    )}</td></tr>`;
  });
  return html + "</table>";
...
export function renderDiagnostics(diagnostics) {
  let html =
    '<div class="diagnostic-box"><h4>🔬 Diagnostic Analysis</h4><ul class="diagnostic-list">';
