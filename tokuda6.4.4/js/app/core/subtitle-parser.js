/**
 * Subtitle Parser
 * Parse YouTube XML subtitles and convert to marked paragraphs
 */

import { CONFIG, SPLIT_DELIMITER } from '../lib/config.js';
import { SeededRandom } from '../lib/helpers.js';
import { countWords } from '../lib/text.js';
import { mergeLowMarkerParagraphs, splitTextAtMiddle } from '../lib/text.js';

const rng = new SeededRandom(CONFIG.RANDOM_SEED);

// ===== Parse XML with Timing =====
export const parseSubtitlesWithTiming = (xml, lang = 'en') => {
  const textElements = Array.from(
    new DOMParser().parseFromString(xml, 'text/xml').getElementsByTagName('text')
  );
