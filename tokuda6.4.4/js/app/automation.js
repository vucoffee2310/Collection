...
(function () {
  ...
  const extractResponseText = () => {
    ...
    // Remove intro text before first marker
  ...
  const monitorResponse = async (expectedMarkerCount) => {
    ...
    while (unchangedChecks < maxUnchangedChecks) {
      ...
      if (currentText && currentText !== previousText) {
        ...
        // Tighten convergence once we have all markers
  ...
  const runAutomation = async (promptText, cardName) => {
    try {
      ...
      if (sliders[0]) setSlider(sliders[0], 1.5);  // Temperature
      await wait(500);
      if (sliders[1]) setSlider(sliders[1], 300);  // Max tokens
      await wait(500);
      if (sliders[2]) setSlider(sliders[2], 0.5);  // Top-p
      await wait(500);
