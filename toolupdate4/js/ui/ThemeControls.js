...
export class ThemeControls {
  ...
  _setupListeners() {
    ...
    // Debounced sliders
    this._setupSlider('opacity-slider', 'opacity-value', val => `${val}%`, () => this._applyOpacity());
    this._setupSlider('brightness-slider', 'brightness-value', val => `${val}%`, () => this._applyBrightness());
    this._setupSlider('spacing-slider', 'spacing-value', val => `${(val/100).toFixed(2)}em`, () => this._updateSpacing());
  }
