...
export class Exporters {
  ...
  _extractOverlayHTML(overlay) {
    ...
    // Read actual pixel values directly
    const styles = [
      `left:${overlay.style.left}`,
      `top:${overlay.style.top}`,
      `width:${overlay.style.width}`,
      `min-height:${overlay.style.minHeight}`,
      `height:auto`,
      `background-color:${overlayStyle.backgroundColor}`,
      `border:1px solid ${overlayStyle.borderColor}`,
      `color:${textStyle.color}`,
      `font-size:${textStyle.fontSize}`
    ].join(';');
    
    return `<div class="overlay" style="${styles}"><span class="overlay-text">${textEl.innerHTML || ''}</span></div>`;
  }
  ...
  async _generatePageHTML(wrapper, pageNum) {
    ...
    const overlaysHTML = Array.from(wrapper.querySelectorAll('.overlay'))
      .map(ov => this._extractOverlayHTML(ov))
      .filter(html => html)
      .join('\n    ');
