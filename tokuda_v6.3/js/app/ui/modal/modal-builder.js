 ...
 * ✅ NEW: Web tab support with check button
 */
...
export const createStreamModal = async (track, getOrCreateJSON, processTrack) => {
  try {
    ...
    console.log('✅ Compound data ready');
  } catch (error) {
    console.warn('⚠️ Compound data not available:', error.message);
  }
...
export const createWebTab = () => {
  const container = document.createElement('div');
  container.id = 'webTab';
  container.style.cssText = 'padding: 16px; background: #f5f5f5; border: 1px solid #ccc;';
  
  container.innerHTML = `
    <div style="margin-bottom: 12px; color: #000; font-size: 12px;">
      Open AI Studio in a new tab and automatically process translation
    </div>
    
    <div id="webTabStatus" style="display: none; padding: 10px; background: white; border: 1px solid #ccc; margin-bottom: 12px; font-size: 11px; font-family: monospace; max-height: 150px; overflow-y: auto;">
      <div style="font-weight: bold; margin-bottom: 5px;">Status:</div>
      <div id="webTabStatusText">Waiting...</div>
    </div>
  `;
  ...
  checkBtn.id = 'webTabCheckBtn';
  checkBtn.textContent = 'Check AI Studio Status';
  checkBtn.style.cssText = `
    padding: 10px 24px; background: white; color: #000;
    border: 1px solid #000; cursor: pointer;
    font-size: 12px; display: none;
  `;
  
  buttonContainer.append(openBtn, checkBtn);
