/**
 * UI Main Entry Point
 * ✅ FIXED: Use POT retry logic + graceful compound data loading
 */
...
const ensureCompoundDataLoaded = async () => {
  if (!compoundDataLoaded) {
    try {
      ...
      console.log('✅ Compound data loaded successfully');
    } catch (error) {
      console.warn('⚠️ Failed to load compound data, continuing without it:', error);
      compoundDataLoaded = true; // Mark as attempted
    }
  }
};
