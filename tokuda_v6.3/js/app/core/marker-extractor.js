 ...
 * ✅ Now calculates overallLength for each marker
 */
...
/**
 * Calculate overall length for a marker instance
 * Uses character count for non-spaced languages, word count for others
 */
const calculateOverallLength = (content, utterances, lang) => {
  if (NON_SPACED_LANGS.has(lang)) {
    // For non-spaced languages: sum of utterance character lengths
    return utterances.reduce((sum, utt) => sum + (utt.utterance?.length || 0), 0);
  } else {
    // For space-separated languages: sum of utterance word counts
    return utterances.reduce((sum, utt) => sum + (utt.wordLength || 0), 0);
  }
};

export const extractMarkersWithContext = (text, utteranceData = [], lang = 'en') => {
  ...
  markers.forEach((m) => {
    ...
    const instance = {
      ...
      overallLength: overallLength,  // ✅ NEW PROPERTY
