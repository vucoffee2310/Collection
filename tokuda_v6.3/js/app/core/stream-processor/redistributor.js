 ...
 * ✅ Stores FULL translation before splitting into portions
 */

import { splitTextIntoWords, splitTranslationByWordRatio, countWordsConsistent } from '../../utils/text/index.js';
import { safelyMergeCompounds, fixCompoundBoundaries } from '../../utils/compounds/index.js';
import { validateWordCountConsistency } from '../../utils/compounds/index.js';
  ...
  Object.values(sourceJSON.markers).forEach(instances => {
    instances.forEach(instance => {
      ...
      if (instance.status === 'MATCHED' && instance.mergedOrphans?.length > 0) {
        redistributeMatchedWithOrphans(instance, sourceJSON, targetLang, sourceLang);
      }
...
/**
 * TWO-PHASE redistribution - stores full translation before splitting
 */
const redistributeMatchedWithOrphans = (instance, sourceJSON, targetLang, sourceLang) => {
  ...
  try {
    ...
    // ===================================================================
    // PHASE 1: Use pre-calculated overallLength
    // ===================================================================
    const forwardLength = forwardMerged.reduce((sum, o) => sum + (o.overallLength || 0), 0);
    const backwardLength = backwardMerged.reduce((sum, o) => sum + (o.overallLength || 0), 0);
    
    const matchedUtterances = instance.utterances.filter(u => !u._mergedFrom);
    const matchedLength = matchedUtterances.reduce((sum, u) => sum + (u.wordLength || 0), 0);
    
    const totalLength = forwardLength + matchedLength + backwardLength;
    
    console.log(`\n📏 PHASE 1: Overall length (from overallLength property):`);
    console.log(`   Forward:  ${forwardLength} (${((forwardLength/totalLength)*100).toFixed(1)}%)`);
    console.log(`   Matched:  ${matchedLength} (${((matchedLength/totalLength)*100).toFixed(1)}%)`);
    console.log(`   Backward: ${backwardLength} (${((backwardLength/totalLength)*100).toFixed(1)}%)`);
    ...
    // ===================================================================
    // Get merged translation (this is the FULL combined translation)
    // ===================================================================
    ...
    // ✅ ROOT FIX: Store FULL translation BEFORE splitting
    instance.fullTranslation = mergedTranslation;
    instance.fullTranslationWithCompounds = mergedTranslation;
    console.log(`   💾 Stored full translation (${countWordsConsistent(mergedTranslation, targetLang)} words)`);
    ...
    // ===================================================================
    // PHASE 1: Split translation by overall length ratio
    // ===================================================================
    const forwardTransWords = Math.round(totalTranslationWords * (forwardLength / totalLength));
    const matchedTransWords = Math.round(totalTranslationWords * (matchedLength / totalLength));
    const backwardTransWords = totalTranslationWords - forwardTransWords - matchedTransWords;
    
    console.log(`\n✂️ PHASE 1 Split: By overall length ratio`);
    console.log(`   Forward:  ${forwardTransWords} words (${((forwardLength/totalLength)*100).toFixed(1)}%)`);
    console.log(`   Matched:  ${matchedTransWords} words (${((matchedLength/totalLength)*100).toFixed(1)}%)`);
    console.log(`   Backward: ${backwardTransWords} words (${((backwardLength/totalLength)*100).toFixed(1)}%)`);
    
    // Split into portions
    let idx = 0;
    let forwardTranslation = translationWords.slice(idx, idx + forwardTransWords).join(' ');
    idx += forwardTransWords;
    
    let matchedTranslation = translationWords.slice(idx, idx + matchedTransWords).join(' ');
    ...
    if (forwardTranslation.trim()) {
      console.log(`   Forward:  ${countWordsConsistent(forwardTranslation, targetLang)} words`);
    }
    if (matchedTranslation.trim()) {
      console.log(`   Matched:  ${countWordsConsistent(matchedTranslation, targetLang)} words`);
    }
    if (backwardTranslation.trim()) {
      console.log(`   Backward: ${countWordsConsistent(backwardTranslation, targetLang)} words`);
    }
    
    // ===================================================================
    // Store PORTIONS to individual instances (for utterance redistribution)
    // ===================================================================
    if (forwardMerged.length > 0 && forwardTranslation.trim()) {
      forwardMerged.forEach(orphan => {
        ...
        if (sourceInstance) {
          ...
          // ✅ Also store full translation for forward orphans (they contributed to it)
    ...
    if (backwardMerged.length > 0 && backwardTranslation.trim()) {
      backwardMerged.forEach(orphan => {
        ...
        if (sourceInstance) {
          ...
          // ✅ Also store full translation for backward orphans
          sourceInstance.fullTranslation = instance.fullTranslation;
          sourceInstance.fullTranslationWithCompounds = instance.fullTranslationWithCompounds;
        }
      });
    ...
    // ===================================================================
    // PHASE 2: Split each portion by utterance word ratios
    // ===================================================================
