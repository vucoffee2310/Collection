...
export class HTMLExporter {
    ...
    async save(name, ui) {
        const ind = ui.showSavingIndicator('Preparing all pages for export...');
        try {
            const font = await this.pdf.loadFont();
            const body = await this._genBody(ind);
            ind.textContent = 'Generating HTML file...';
            this._dl(this._buildHTML(name, font, body), `${name}_view.html`);
        } catch (e) {
