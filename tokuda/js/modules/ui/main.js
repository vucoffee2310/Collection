...
const createTrackRow = (track) => {
  ...
  actions.append(
    btn('SSE Simulator', async (e) => {
      e.preventDefault();
      const modal = createStreamModal(track, getOrCreateJSON);
      document.body.append(modal);
      modal.querySelector('textarea').focus();
    }, true),
    
    btn('Send to AI', async (e) => {
      e.preventDefault();
      const b = e.target;
      ...
      b.disabled = true;
      const result = await processTrack(track);
      if (result) await sendToAI(result.content);
      b.textContent = 'Sent ✓';
      setTimeout(() => { b.textContent = orig; b.disabled = false; }, 2000);
    }),
    
    btn('Copy Text', async (e) => {
      e.preventDefault();
      const result = await processTrack(track);
      if (result) {
