 ...
 * ✅ DEFINITIVE FIX: Removed faulty `previousText` from update message. The modal now handles its own state for diffing.
 */
...
(function() {
  ...
  const extractResponseText = () => {
    ...
    // Aggressively find the first marker and strip any preceding text.
    ...
    if (firstMarkerMatch) {
      ...
      if (markerIndex > 0) {
        ...
        if (introText.length < 200) { // Safety check
  ...
  const monitorResponse = async (expectedMarkerCount) => {
    ...
    while (unchangedChecks < maxUnchangedChecks) {
      ...
      if (currentText && currentText !== previousText) {
        ...
        // ✅ ROOT FIX: Send ONLY the current sanitized text. Do NOT send previousText.
