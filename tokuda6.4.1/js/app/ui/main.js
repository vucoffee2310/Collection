...
export const createUI = (tracks) => {
  ...
  if (!tracks?.length) {
    container.innerHTML = '<div style="padding: 10px; background: #f0f0f0; border: 1px solid #ccc; color: #000; font-size: 12px; border-radius: 4px;">No subtitles available</div>';
  } else {
    const title = document.createElement('div');
    title.style.cssText = 'font-size: 14px; font-weight: bold; color: #000; margin-bottom: 10px;';
    title.textContent = `Translation Processor (${tracks.length} subtitle${tracks.length > 1 ? 's' : ''})`;
