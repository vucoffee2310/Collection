...
export const createProcessorUI = (track) => {
  ...
  container.style.cssText = 'padding: 12px; background: #f5f5f5; border: 1px solid #ccc; margin: 8px 0; border-radius: 4px;';
  ...
  statsBar.style.cssText = 'display: none; padding: 10px; background: #e3f2fd; border: 1px solid #2196F3; border-radius: 4px; margin-bottom: 8px; font-size: 11px;';
  ...
  const renderPosition = (item, isNew = false) => {
    ...
    const jsonObj = {
      ...
      ...(item.overallTranslation && { translation: item.overallTranslation.substring(0, 100) }),
      ...
      const handler = (e) => {
        ...
        if (msg.action === 'aiStudioUpdate') {
          ...
          if (msg.isComplete) {
            ...
            const rate = sourceJSON.totalMarkers ? (((processor.stats.matched + processor.stats.merged) / sourceJSON.totalMarkers) * 100).toFixed(1) : 0;
  ...
  style.textContent = '@keyframes slideIn { from { opacity: 0; transform: translateX(-5px); } to { opacity: 1; transform: translateX(0); } }';
