...
export class SplitModal {
  ...
  async handleExport() {
    ...
    const actions = {
      ...
      'by-files': () => this.pdfExp.splitByNumberOfFiles(
        parseInt(el('num-files').value || 2),
        this.pdfName,
        this.pageManager
      ),
      'by-pages': () => this.pdfExp.splitByPagesPerFile(
        parseInt(el('pages-per-file').value || 10),
        this.pdfName,
        this.pageManager
      ),
      range: () => this.pdfExp.exportPageRange(
        parseInt(el('range-start').value || 1),
        parseInt(el('range-end').value || 1),
        this.pdfName,
        this.pageManager
      ),
      custom: () => this.pdfExp.exportSpecificPages(
