...
export class OverlayRenderer {
    ...
    renderPageOverlays(pageWrapper, pageNum, dimensions, overlayData) {
        ...
        const expansion = this.stateManager.expansion;

        Object.keys(pageData).forEach(coords => {
            fragment.appendChild(this._createOverlay(coords, pageData[coords], pageNum, W, H, expansion));
        });
        
        pageWrapper.appendChild(fragment);
    }
    
    _createOverlay(coords, info, pageNum, W, H, expansion) {
        if (W <= 0 || H <= 0) return document.createElement('div');
        const pageKey = `page_${pageNum}`;

        const pos = Utils.calculateOverlayPosition({
            coords, pageKey, expansion,
