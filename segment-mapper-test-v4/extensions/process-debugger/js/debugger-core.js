...
export class ProcessDebugger {
    ...
    analyzeWaitingMarker(marker) {
        console.log(`[Process Debugger] 📊 Analyzing waiting marker: ${marker}`);

        const source = this.sourceSegments.find((s) => s.marker === marker);
        if (!source) {
            console.warn(`[Process Debugger] Marker ${marker} not found in sources`);
            return;
        }

        // Only create waiting event if it doesn't already exist
    ...
    handleRaceCondition(data) {
        console.log("[Process Debugger] 🔴 Race condition detected:", data);

        const timestamp = new Date().toLocaleTimeString();
        const eventData = createRaceConditionEvent(data, timestamp, this);

        this.storeEvent(data.marker, eventData);
        this.eventCount++;
        this.throttledStatsUpdate();

        // Update breakdown to reflect the race condition
    ...
    renderMarkerBreakdown() {
        ...
        const gridHtml = `<div class="breakdown-grid">${allMarkers.map(item =>
    ...
    renderSteps(steps) {
        return steps.map((step, i) => {
            ...
            const contentHtml = `<div class="step-content">${step.content}${step.code ? `<div class="code-box">${escapeHtml(step.code)}</div>` : ''
