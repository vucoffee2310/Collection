document.addEventListener('DOMContentLoaded', () => {
    ...
    createFFTModule().then(Module => {
                ...
                if (fmt.bitsPerSample === 16) {
                    ...
                    readFn = pos => {
                        const b0 = dv2.getUint8(pos),
                            b1 = dv2.getUint8(pos + 1),
                            b2 = dv2.getUint8(pos + 2);
        ...
        async function loadAudio(file, targetSr) {
            ...
            if (parsed) {
                const out = resampleLinear(parsed.samples, parsed.sampleRate, targetSr, true);
                return {
                    samples: out,
        ...
        function findPeaks(x, height, distance) {
            ...
            if (!n) return [];
            const peaks = [];
            if (n > 1 && x[0] > x[1] && x[0] >= height) peaks.push({
                idx: 0,
                val: x[0]
            });
            for (let i = 1; i < n - 1; i++) {
                const v = x[i];
                if (v >= height && v > x[i - 1] && v > x[i + 1]) peaks.push({
                    idx: i,
                    val: v
                })
            }
            if (n > 1 && x[n - 1] > x[n - 2] && x[n - 1] >= height) peaks.push({
                idx: n - 1,
                val: x[n - 1]
            });
        ...
        function calculateNCC(search, pattern) {
            ...
            const pMean = ps / M,
                pVar = Math.max(0, psq / M - pMean * pMean),
                pEnergy = Math.sqrt(pVar * M) || 0;
            ...
            for (let i = 0; i <= n1 - M; i++) {
                if (i) {
                    const add = search[i + M - 1],
                        rem = search[i - 1];
                    ss += add - rem;
                    ssq += add * add - rem * rem
                }
                const mean = ss * Minv,
                    v = Math.max(0, ssq * Minv - mean * mean),
                    denom = Math.sqrt(v * M) * pEnergy;
        ...
        runButton.addEventListener('click', async () => {
            ...
            try {
                ...
                const [patPack, seaPack] = await Promise.all([loadAudio(patternFile, target_sr), loadAudio(searchFile, target_sr)]);
                const pattern = patPack.samples,
                    search = seaPack.samples;
                ...
                if (!peaks.length) {
                    ...
                    for (let i = 0; i < peaks.length; i++) {
                        ...
                        const start = p / target_sr,
                            end = (p + pattern.length) / target_sr;
